...
import { updateOrder } from "./select";
...
function UpdateSelect({
  ...
  useEffect(() => {
    setSelectUpdateOrder(searchState.date);
  }, [searchState.date]);
  return (
    <>
      <div className=" md:w-48 sm:w-36 w-28 ">
        <p className="sm:text-sm text-xs">更新順：</p>
        <div className="block relative">
          <select
            value={selectUpdateOrder}
            onChange={(e) => setSelectUpdateOrder(e.target.value)}
